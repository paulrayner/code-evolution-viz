...
export class TreeVisualizer {
    constructor(canvas) {
        ...
        this.colorMode = 'fileType';
        this.highlightedFiles = new Set();
        this.highlightedFileTypes = new Map(); // Track type of highlighted files
        this.deletedFileNodes = new Map(); // Store metadata for deleted files (ghost nodes)
        this.timelineMode = 'off'; // Timeline mode: v1=spotlight, v2=additive
        this.filterManager = new FilterManager(); // Filter manager for HEAD view only
        this.viewMode = 'navigate'; // View mode: navigate=depth-limited, overview=show all
        // Ghost rendering for Timeline V2 deletions (isolated for future replacement)
        this.ghostMeshes = new Set();
        this.ghostEdges = new Set();
        this.edges = [];
        this.edgeNodeMap = new Map();
        // Scene setup
    ...
    updateNodeVisibility() {
        ...
        for (const layoutNode of this.layoutNodes) {
            if (layoutNode.mesh) {
                ...
                const material = layoutNode.mesh.material;
                ...
                if (layoutNode.node.type === 'directory') {
                    const label = layoutNode.mesh.children.find(child => child instanceof CSS2DObject);
                    if (label && label.element instanceof HTMLDivElement) {
     ...
     */
    highlightFilesByType(addedFiles, modifiedFiles, deletedFiles = [], deletedFileNodes = []) {
    ...
    updateHighlighting() {
        ...
        for (const edge of this.edges) {
            ...
            const childPath = childIsFile ? edgeInfo.child.path : '';
     ...
     */
    calculateVerticalSpacing(level) {
     ...
     */
    layoutTree(node, position, level, angleStart, angleEnd, parentLayout) {
        const nodes = [];
        const currentLayout = { node, position, parent: parentLayout };
        ...
        node.children.forEach((child, index) => {
            ...
            const childPosition = new THREE.Vector3(position.x + Math.cos(angle) * radius, position.y - verticalSpacing, position.z + Math.sin(angle) * radius);
     ...
     */
    isAncestorOfFocused(layoutNode) {
     ...
     */
    isSiblingOfAncestor(layoutNode) {
        if (!this.focusedDirectory)
            return false;
        // Find the focused layout node
        const focusedLayout = this.layoutNodes.find(ln => ln.node === this.focusedDirectory);
     ...
     */
    createVisuals(layoutNodes, maxFileLoc, maxDirLoc) {
    ...
    autoFrameCamera(boundingBox) {
        ...
        this.camera.position.set(center.x + cameraDistance * Math.cos(angle), center.y + cameraDistance * 0.5, center.z + cameraDistance * Math.sin(angle));
        // Look at center
     ...
     */
    visualize(tree, resetCamera = true) {
                ...
                if (layoutNode) {
                    ...
                    while (current) {
                        if (current.mesh && current.mesh !== this.selectedObject) {
                            const mat = current.mesh.material;
                            mat.emissiveIntensity = 0.6;
                            this.hoveredObjects.add(current.mesh);
                            // Show label if in hover mode
                            if (this.labelMode === 'hover') {
                                const label = current.mesh.children.find(child => child instanceof CSS2DObject);
                                if (label && label.element instanceof HTMLDivElement) {
                                    label.element.style.visibility = 'visible';
                                    label.element.style.display = 'block';
                                }
     ...
     */
    highlightDescendants(layoutNode) {
        if (layoutNode.node.type === 'directory') {
            const dirNode = layoutNode.node;
            ...
            for (const child of dirNode.children) {
                ...
                if (childLayout && childLayout.mesh && childLayout.mesh !== this.selectedObject) {
                    const mat = childLayout.mesh.material;
                    mat.emissiveIntensity = 0.6;
        ...
        if (intersects.length > 0) {
            ...
            if (dir) {
                ...
                if (this.selectedObject) {
                    const mat = this.selectedObject.material;
     ...
     */
    calculateGhostPosition(parentMesh, parentDirNode) {
            ...
            if (child.type === 'file') {
                for (const [mesh, fileNode] of this.fileObjects.entries()) {
                    if (fileNode.path === child.path) {
                        siblingMeshes.push(mesh);
                        if (siblingMeshes.length === 2)
                            break;
                    }
                }
                if (siblingMeshes.length === 2)
        ...
        if (siblingMeshes.length === 0) {
            ...
            return new THREE.Vector3(parentPos.x + defaultRadius * Math.cos(randomAngle), parentPos.y, parentPos.z + defaultRadius * Math.sin(randomAngle));
        }
        ...
        if (siblingMeshes.length === 1) {
            ...
            return new THREE.Vector3(parentPos.x + radius * Math.cos(oppositeAngle), siblingMeshes[0].position.y, // Use same Y as sibling
        ...
        return new THREE.Vector3(parentPos.x + avgRadius * Math.cos(midAngle), avgY, // Use average Y of siblings
     ...
     */
    renderDeletionGhosts(deletedPaths, prevTree) {
     ...
     */
    findFileInPrevTree(tree, targetPath) {
        const traverse = (node) => {
            if (node.type === 'file') {
     ...
     * @param prevTree - Previous tree state (where files still exist)
     */
    renderDeletedFiles(deletedPaths, prevTree) {
