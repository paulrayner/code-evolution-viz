...
- [ ] **Generated/Minified File Detection** (Multi-phase implementation)
  ...
  **Phase 1.6: Expand Pattern Coverage - Common Generated Files** ✅ COMPLETE (2025-10-26)
  - [x] Add lock file patterns (yarn.lock, package-lock.json, pnpm-lock.yaml, etc.)
  - [x] Add source map pattern (.map files)
  - [x] Add Jest snapshot patterns (__snapshots__/, .snap)
  - [x] Add compiled test fixture pattern (__compiled__/)
  - [x] Update isGeneratedFile() in processor/src/analyze.ts
  - [x] Re-analyze React repository to verify new detection count
  - [x] Updated viewer data with re-analyzed React repository

  **Implementation:** Added 12 new patterns to isGeneratedFile():
  - Lock files (8): yarn.lock, package-lock.json, pnpm-lock.yaml, composer.lock, Gemfile.lock, Cargo.lock, poetry.lock, Pipfile.lock
  - Build artifacts (1): .map (source maps)
  - Test artifacts (3): /__snapshots__/, .snap, /__compiled__/

  **Results:**
  - React repository: **10 → 288 generated files detected** (28.8× improvement)
  - Breakdown: 44 lock files + 68 .map files + 20 snapshot files + 211 __compiled__ files + 10 .min.js files = ~353 total patterns matched (some files deduplicated to 288)
